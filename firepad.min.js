...
function(a, b, c) {
	...
	a.utils = {}, a.utils.makeEventEmitter = function(a, b) {
		a.prototype.allowedEvents_ = b, a.prototype.on = function(a, b, c) {
			...
			for (var c = this.eventListeners_[a] || [], d = 0; d < c.length; d++) if (c[d].callback === b) return void c.splice(d, 1)
			...
			if (this.allowedEvents_) {
				for (var b = !1, c = 0; c < this.allowedEvents_.length; c++) if (this.allowedEvents_[c] === a) {
					b = !0;
					break
				}
		...
		return b.prototype.equals = function(a) {
			...
			for (var e, g, h = [], i = 0, j = 0, k = this.ops, l = 0, m = k.length; m > l; l++) {
				...
				if (n.isRetain()) {
					...
					j += n.chars
				} else if (n.isInsert()) for (h[i++] = n.text, e = 0; e < n.text.length; e++) {
					var q = {};
	...
	a.AnnotationList = function() {
		...
		return e.prototype.insertAnnotatedSpan = function(a, c) {
			...
			if (a) for (var b = null, c = a; c;) b && b.annotation.equals(c.annotation) ? (b.length += c.length, b.next = c.next) : b = c, c = c.next
	...
	a.FirebaseAdapter = function(b) {
		function c(a, b, c) {
			...
			if (b) {
				...
				this.firebaseOn_(e, "value", function(a) {
		...
		void 0 === window.firebase && "function" == typeof require && "function" != typeof Firebase && (firebase = require("firebase"));
	...
	a.RichTextCodeMirror = function() {
		...
		return b.prototype.detach = function() {
			...
			for (var c = a.length; c--;) for (var d = a[c], e = b.length; e--;) {
				var f = b[e];
			...
			if (b > -1) for (var c = this.codeMirror.lineCount(); c > b + 1 && this.lineIsListItemOrIndented_(b + 1);) b++;
			for (var d = [], e = this.codeMirror, f = a; b >= f; f++) {
				var g = e.getLine(f),
					h = e.getLineHandle(f);
				if (e.removeLineClass(h, "text", ".*"), g.length > 0) for (var i = g.indexOf(r); i >= 0;) {
	...
	a.RichTextCodeMirrorAdapter = function() {
		...
		return b.prototype.detach = function() {
			...
			for (var b = a.ops, c = 0, d = 0, e = b.length; e > d; d++) {
				var f = b[d];
				f.isRetain() ? (h(f.attributes) || this.rtcm.updateTextAttributes(c, c + f.chars, function(a) {
					for (var b in f.attributes) f.attributes[b] === !1 ? delete a[b] : a[b] = f.attributes[b]
				}, "RTCMADAPTER", !0), c += f.chars) : f.isInsert() ? (this.rtcm.insertText(c, f.text, f.attributes, "RTCMADAPTER"), c += f.text.length) : f.isDelete() && this.rtcm.removeText(c, c + f.chars, "RTCMADAPTER")
			}
			a.ops.length > 10 && (this.rtcm.codeMirror.getWrapperElement().setAttribute("style", ""), this.rtcm.codeMirror.refresh())
			...
			for (var b, c, d = 0, e = this.rtcm.codeMirror, f = new k, g = 0; g < a.wrapped.ops.length; g++) {
				...
				if (i.isRetain()) if (h(i.attributes)) f.retain(i.chars), d += i.chars;
				else for (b = this.rtcm.getAttributeSpans(d, d + i.chars), c = 0; c < b.length; c++) {
					var j = {};
	...
	a.Headless = function() {
		function b(a) {
			...
			var c, f;
			"string" == typeof a ? (void 0 === window.firebase && "object" != typeof c ? (console.log("REQUIRING"), c = require("firebase")) : c = window.firebase, f = c.database().refFromURL(a)) : f = a, this.entityManager_ = new e, this.firebaseAdapter_ = new d(f), this.ready_ = !1, this.zombie_ = !1
		}
